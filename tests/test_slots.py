...
T9 = datetime(2030, 1, 1, 9, 0, tzinfo=timezone.utc)


def post_slot(client, start, end, clinician_id="dr-1"):
    return client.post("/slots", json={"clinician_id": clinician_id, "start": start, "end": end})
...
def test_offset_normalized_to_utc(client):
    response = post_slot(client, "2030-01-01T11:00:00+02:00", "2030-01-01T11:30:00+02:00")
    assert response.status_code == 201
    body = response.json()
    assert parse(body["start"]) == T9
...
def test_missing_field_rejected(client):
    response = client.post("/slots", json={"start": "2030-01-01T09:00:00Z", "end": "2030-01-01T09:30:00Z"})
...
@pytest.mark.parametrize(("minutes", "expected"), [(14, 422), (15, 201), (120, 201), (121, 422)])
def test_duration_bounds(client, minutes, expected):
    start = T9 + timedelta(hours=1)
    response = post_slot(client, start.isoformat(), (start + timedelta(minutes=minutes)).isoformat())
    assert response.status_code == expected


def test_past_start_rejected(client):
    start = FIXED_NOW - timedelta(minutes=1)
    response = post_slot(client, start.isoformat(), (start + timedelta(minutes=30)).isoformat())
    assert response.status_code == 422


def test_start_equal_to_now_accepted(client):
    response = post_slot(client, FIXED_NOW.isoformat(), (FIXED_NOW + timedelta(minutes=30)).isoformat())
    assert response.status_code == 201
...
def test_identical_slot_overlaps(client):
    ...
    assert post_slot(client, "2030-01-01T09:00:00Z", "2030-01-01T09:30:00Z").status_code == 409


@pytest.mark.parametrize(
    ("start", "end"),
    [("2030-01-01T09:30:00Z", "2030-01-01T10:00:00Z"), ("2030-01-01T08:30:00Z", "2030-01-01T09:00:00Z")],
)
...
def test_different_clinicians_may_overlap(client):
    ...
    assert post_slot(client, "2030-01-01T09:00:00Z", "2030-01-01T09:30:00Z", clinician_id="dr-2").status_code == 201
...
def test_validation_takes_precedence_over_overlap(client):
    ...
    assert post_slot(client, "2030-01-01T09:00:00Z", "2030-01-01T09:10:00Z").status_code == 422
...
def test_list_sorted_by_start(client):
    for hour in (10, 8, 9):
        make_slot(client, datetime(2030, 1, 2, hour, tzinfo=timezone.utc), clinician_id=f"dr-{hour}")
...
def test_filter_by_utc_date(client):
    ...
    response = post_slot(client, "2030-01-02T01:00:00+03:00", "2030-01-02T01:30:00+03:00")
    slot_id = response.json()["id"]
    make_slot(client, datetime(2030, 1, 2, 9, tzinfo=timezone.utc))
...
def test_filter_by_availability(client):
    ...
    assert [s["id"] for s in client.get("/slots", params={"available": "true"}).json()] == [free["id"]]
    assert [s["id"] for s in client.get("/slots", params={"available": "false"}).json()] == [booked["id"]]
...
def test_combined_filters(client):
    ...
    body = client.get("/slots", params={"clinician_id": "dr-1", "date": "2030-01-01", "available": "true"}).json()
    assert len(body) == 1
    assert parse(body[0]["start"]) == T9 + timedelta(hours=1)


@pytest.mark.parametrize("params", [{"date": "01-01-2030"}, {"date": "2030-13-01"}, {"available": "maybe"}])
